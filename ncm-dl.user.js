...
(function () {
    ...
    function saveContentAsFile(content, fileName) {
    ...
    function createBigButton(desc, parent, appendWay) {
    ...
    function combineLyric(lyricOri, lyricAdd) {
        let resLyric = {
            lyric: '',
    ...
    function parsedLyricsBinarySearch(lyric, lyrics) {
        let time = lyric.time;
    ...
    function trimLyricContent(content) {
        let t = content.trim();
        return t.length < 1 ? content : t;
    }
    function handleLyric(lyricRes) {
        let LyricObj = {
            orilrc: {
    ...
    if (location.href.includes('song')) {
        ...
        if (cvrwrap) {
            ...
            let songDetailAppend = new SongDetailAppend(songId, cvrwrap)

            songDetailAppend.start()
    ...
    if (location.href.includes('user')) {
        ...
        if (editArea && urlUserId == unsafeWindow.GUser.userId) {
            //听歌量打卡
            let btnListen = createBigButton('听歌量打卡', editArea, 2)
            ...
            //歌曲快传
            let btnUpload = createBigButton('快速上传加载中', editArea, 2)
...
`,
            ...
            //匹配纠正
            let btnMatch = createBigButton('云盘匹配纠正', editArea, 2)
...
`,
            ...
            //音质升级
            let btnUpgrade = createBigButton('云盘音质提升', editArea, 2)
...
`,
            ...
            //本地上传
            let btnLocalUpload = createBigButton('云盘本地上传', editArea, 2)
...
`,
            ...
            //限免VIP歌曲
            let btnVIPfreeA = createBigButton('限免VIP歌曲A', editArea, 2)
            ...
            let btnVIPfreeB = createBigButton('限免VIP歌曲B', editArea, 2)
...
`,
            ...
            //云盘导出
            let btnExport = createBigButton('云盘导出', editArea, 2)
            ...
            //云盘导入
            let btnImport = createBigButton('云盘导入', editArea, 2)
    ...
    if (location.href.includes('playlist') || location.href.includes('album')) {
        ...
        if (operationArea && unsafeWindow.GUser.userId) {
            //批量下载
            let btnBatchDownload = createBigButton('批量下载', operationArea, 1)
...
<label>同时下载的歌曲数<select id="thread-count-select" class="swal2-select"><option value=4 selected="">4</option><option value=3>3</option><option value="2">2</option><option value=1>1</option></select></label>
    ...
    `,
            ...
            //批量转存云盘
            let btnBatchUpload = createBigButton('批量转存云盘', operationArea, 1)
...
`,
            ...
            function downloadSongLyric(songId, fileName) {
            ...
            if (pageType == 'playlist') {
                ...
                if (creatorId == unsafeWindow.GUser.userId) {
                    //歌单排序
                    let btnPlaylistSort = createBigButton('歌单排序', operationArea, 1)
            ...
            //显示脏标
                ...
                let topblk = document.querySelector('.topblk')
                if (topblk) {
                    weapiRequest(`/api/v1/album/${listId}`, {
                        onload: (content) => {
                            console.log(content)
                            topblk.innerHTML += `<p class="intr"><b>专辑类型：</b>${content.album.type} ${content.album.subType}</p>`
                            if ((content.album.mark & songMark.explicit) == songMark.explicit) {
                                topblk.innerHTML += `<p class="intr"><b>🅴：</b>内容含有不健康因素</p>`
                            }
                            if (content.album.blurPicUrl) {
                                topblk.innerHTML += `<p class="intr"><a class="s-fc7" href="${content.album.blurPicUrl}" target="_blank">专辑封面原图</a></p>`
                            }
                        }
                    })
                }
