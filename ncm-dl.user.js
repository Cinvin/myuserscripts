...
`,
                        ...
                        didOpen: () => {
                            ...
                            this.popupObj={container:container,tbody:tbody,footer:footer}
                            ...
                            this.btnUploadBatch.addEventListener('click', () => {
                                if(this.batchUpload.working){ return }
                ...
                fetchSongInfo(){
                    ...
                    let ids=this.config.data.map(item=>{ return {'id':item.id} })
                    //获取需上传的song
                ...
                fetchSongInfoSub(ids,startIndex){
                    ...
                    weapiRequest("/api/v3/song/detail", {
                        ...
                        method: "post",
                        sync: true,
                        data: {c: JSON.stringify(ids.slice(startIndex,startIndex+1000))},
                        onload: function(responses) {
                            let content=JSON.parse(responses.response)
                            //console.log(content)
                            ...
                            for(let i =0;i<len;i++){
                                if(!content.privileges[i].cs){
                                    let config=uploader.config.data.find(item=>{return item.id==content.songs[i].id})
                                    let item={
                ...
                renderData(){
                    ...
                    if(this.page.max>1){pageIndexs.push(this.page.max)}
                    let uploader=this
                    this.popupObj.footer.innerHTML=''
                ...
                renderFilterInfo(){
                    ...
                    if(countCanUpload>0) {this.btnUploadBatch.innerHTML+=` (${countCanUpload}首 ${fileSizeDesc(sizeTotal)})`}
                }
            ...
            class Matcher{
                start(){
                    ...
                    this.currentPage=1
                    this.filter={text:'',notMatch:false,songs:[],filterInput:null,notMatchCb:null}
                    this.controls={tbody:null,pageArea:null,cloudDesc:null}
                    this.openCloudList()
                }
                openCloudList(){
                    Swal.fire({
...
`,
                        ...
                        didOpen: () => {
                            ...
                            let matcher=this
                            filterInput.addEventListener('change', () => {
                                if(matcher.filter.text==filterInput.value.trim()){ return }
                ...
                fetchCloudInfoForMatchTable(offset){
                    ...
                    weapiRequest('/api/v1/cloud/get', {
                        ...
                        let floor = Math.max(2,matcher.currentPage-2);
                            ...
                            if(maxPage>1){pageIndexs.push(maxPage)}
                            ...
                            res.data.forEach(song=>{
                                if(matcher.filter.text.length>0){
                                    ...
                                    if(!matchFlag&&song.simpleSong.ar){
                                        song.simpleSong.ar.forEach(ar=>{
                                            if(ar.name&&ar.name.includes(matcher.filter.text)) { matchFlag=true }
                                        })
                                        if (!matchFlag){
                                            return
                                        }
                    ...
                    onload: (responses)=>{
                        ...
                        res.data.forEach(song=>{
                            if(song.simpleSong.al&&song.simpleSong.al.id>0){
                                ...
                                if(filter[1].length>0 && filter[1]!=song.simpleSong.al.name){return}
                                if(filter[2].length>0 && filter[2]!=song.simpleSong.name){return}
                                let songItem = {
                                    'id': song.songId,
                                    'size': song.fileSize,
                                ...
                                //未关联歌曲
                                if(filter[0].length>0&&song.artist!=filter[0]){ return }
                                if(filter[1].length>0&&song.album!=filter[1]){ return }
                                if(filter[2].length>0&&song.songName!=filter[2]){ return }
                                let songItem = {
                                    'id': song.songId,
                                    'size': song.fileSize,
                                    'ext': song.fileName.split('.').pop().toLowerCase(),
