  ...
  class CloudDiskManager {
    ...
    updateFilterButtonText() {
      if (this.controls.filterToggleBtn) {
        const isDefault =
          this.filter.text === '' &&
          this.filter.matchStatus === 'all' &&
          this.filter.pureMusic === 'all' &&
    ...
    checkSongMatchesFilters(song) {
      ...
      if (this.filter.text.length > 0) {
        let matchFlag = false;
        if (
          song.album.includes(this.filter.text) ||
          song.artist.includes(this.filter.text) ||
          song.simpleSong.name.includes(this.filter.text) ||
          (song.simpleSong.al &&
            song.simpleSong.al.id > 0 &&
            song.simpleSong.al.name &&
            song.simpleSong.al.name.includes(this.filter.text))
        ) {
          matchFlag = true;
        }
      ...
      if (this.filter.pureMusic !== 'all') {
        ...
        const isPureMusic =
          (song.simpleSong.mark & 131072) === 131072 ||
          titleLower.includes('伴奏') ||
          titleLower.includes('纯音乐') ||
            ...
            if (this.filter.allSongs && this.filter.allSongs.length > 0 && res.matchData) {
              for (let i = 0; i < this.filter.allSongs.length; i++) {
                if (this.filter.allSongs[i].simpleSong && this.filter.allSongs[i].simpleSong.id == fromId) {
                  res.matchData.simpleSong.privilege =
                    this.filter.allSongs[i].simpleSong.privilege || res.matchData.simpleSong.privilege;
    ...
    fillSearchTable(searchContent, cloudSongId, matchId = null) {
      if (searchContent.result.songCount > 0) {
        ...
        searchContent.result.songs.forEach((resultSong) => {
          ...
          } else if (Math.abs(resultSong.dt - this.fileDuringTime) < 1000) timeMatchSongs.push(resultSong);
    ...
    toggleBatchPanel() {
      ...
      if (this.controls.tbody) {
        ...
        const filterPanelH = this.controls.filterPanel.classList.contains('show')
          ? this.controls.filterPanel.offsetHeight
          : 0;
    ...
    _handleCloudListOpen(cloudListContainer, cloudListFooter) {
      ...
      this.controls.filterToggleBtn.addEventListener('click', () => {
        ...
        if (isShow) {
          ...
          cloudListContainer.querySelector(`input[name="match-status"][value="${this.filter.matchStatus}"]`).checked =
            true;
          cloudListContainer.querySelector(`input[name="pure-music"][value="${this.filter.pureMusic}"]`).checked = true;
          cloudListContainer.querySelector(`input[name="live-version"][value="${this.filter.liveVersion}"]`).checked =
            true;
        } else {
          filterIconEl.className = 'fa-solid fa-arrow-down filter-icon';
        }
    ...
    _handleMatchPopupOpen(song, container, actions, footer, title) {
      ...
      this.searchBtn.addEventListener('click', () => {
        ...
        if (URLObj && URLObj.hostname === 'music.163.com') {
          const urlParamsStr =
            URLObj.search.length > 0 ? URLObj.search : URLObj.href.slice(URLObj.href.lastIndexOf('?'));
        ...
        if (requestData['/api/cloudsearch/get/web'] || requestData['/api/v3/song/detail']) {
          ...
          weapiRequest('/api/batch', {
            ...
            onload: (content) => {
              ...
              const matchId =
                songDetailContent && songDetailContent.songs && songDetailContent.songs.length > 0
                  ? songDetailContent.songs[0].id
                  : null;
