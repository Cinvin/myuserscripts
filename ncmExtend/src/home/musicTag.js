                ...
                onload: (content) => {
                  ...
                  if (searchContent.result.songCount > 0) {
                    ...
                    searchContent.result.songs.forEach((resultSong) => {
                      ...
                      } else if (Math.abs(resultSong.dt - file.duration) < 1000) timeMatchSongs.push(resultSong);
                    ...
                    resultSongs.forEach((resultSong) => {
                      ...
                      const artists = resultSong.ar
                        .map(
                          (ar) =>
                            `<a href="https://music.163.com/#/artist?id=${ar.id}" target="_blank">${escapeHtml(ar.name)}</a>`
                        )
                        .join();
    ...
    refreshSongListTable() {
      ...
      this.fileList.forEach((item) => {
        ...
        if (item.mode === 'netease' && item.targetSong) {
          ...
          const coverHtml = item.customSong.cover
            ? `<img src="${item.customSong.cover.url}" height=50 title="${escapeHtml(item.customSong.album)}">`
            : '';
        ...
        if (file.mode === 'unfill') {
          ...
          if (response && response.code == 200 && response.result.songCount > 0) {
            for (const resultSong of response.result.songs) {
              if (
                Math.abs(resultSong.dt - file.duration) < 1000 &&
                searchWord.toLowerCase().includes(resultSong.name.toLowerCase())
              ) {
                file.targetSong = resultSong;
                file.mode = 'netease';
                this.refreshSongListTable();
