...
export const scriptSettings = (uiArea) => {
  ...
  function openDownloadSettingPopup() {
    Swal.fire({
      ...
      footer:
        '<div>建立文件夹功能篡改猴下载模式设置为浏览器API可以生效，其他脚本管理器可能导致文件名乱码的问题。</div>',
      preConfirm: () => {
...
export const setPlayLevel = () => {
  Swal.fire({
    ...
    showCloseButton: true,
    footer:
      '<a href="https://github.com/Cinvin/myuserscripts"  target="_blank"><img src="https://img.shields.io/github/stars/cinvin/myuserscripts?style=social" alt="Github"></a>',
  }).then((result) => {
    if (result.isConfirmed) {
