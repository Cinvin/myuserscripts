...
export class ncmDownUpload {
    ...
    try {
      weapiRequest(song.api.url, {
        ...
        onload: (content) => {
          ...
          if (resData.url !== null) {
            song.fileFullName =
              nameFileWithoutExt(song.title, song.artist, 'artist-title') + '.' + resData.type.toLowerCase();
  ...
  uploadNextSong() {
    ...
    if (this.currentIndex < this.songs.length) {
      ...
      msg =
        this.songs.length > 1
          ? this.failSongs.length === 0
            ? '全部上传完成'
            : `上传完毕,存在${this.failSongs.length}首上传失败的歌曲.它们为:${this.failSongs.map((song) => song.title).join()}`
          : msg;
