...
const downloadSongSub = (threadIndex, songList, config) => {
  ...
  if (!song.api) {
    song.api =
      ...
      (levelWeight[song.privilege.plLevel] || 99) < (levelWeight[song.privilege.dlLevel] || -1)
    ...
    onload: (content) => {
      ...
      if (albumContent) {
        songItem.albumDetail = {
          ...
          artists: albumContent.album.artists
            ? albumContent.album.artists.map((artist) => artist.name).join('; ')
            : null,
