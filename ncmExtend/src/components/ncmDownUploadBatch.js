...
export const importAPIDataLimit = 10;
export class ncmDownUploadBatch {
  constructor(songs, config) {
    ...
    for (let i = 0; i < songs.length; i++) {
      ...
      if (!songs[i].api) {
        songs[i].api =
          songs[i].privilege.fee === 0 &&
          (levelWeight[songs[i].privilege.plLevel] || 99) < (levelWeight[songs[i].privilege.dlLevel] || -1)
            ? {
  ...
  startUpload() {
    Swal.fire({
      ...
      footer:
        '<div>建议先在个人主页先设置好请求头，以避免上传失败</div><div>浏览器F12控制台中可查看所有的接口返回内容，出错时可进行检查。</div>',
      didOpen: () => {
      ...
      onload: (content) => {
        ...
        content.data.forEach((songFileData) => {
          ...
          if (this.config.targetLevelOnly && this.config.level !== songFileData.level) {
            ...
            this.songs[songIndex].fileFullName =
              nameFileWithoutExt(this.songs[songIndex].title, this.songs[songIndex].artist, 'artist-title') +
              '.' +
              songFileData.type.toLowerCase();
