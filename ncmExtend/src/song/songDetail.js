...
class SongDetail {
      ...
      if (this.SongRes['/api/v3/song/detail'].privileges[0].cs) {
        ...
        if (songDlWeight > songPlWeight && this.SongRes['/api/v3/song/detail'].privileges[0].fee === 0) {
          ...
          if (songDetail.hr && songDlWeight >= 5 && songPlWeight < 5) {
            const desc = `${Math.round(songDetail.hr.br / 1000)}k\t${fileSizeDesc(songDetail.hr.size)}\t${songDetail.hr.sr / 1000}kHz`;
            const level = 'hires';
            this.createDLRow(desc, level, channel);
            this.createULRow(desc, level, channel);
          }
          if (songDetail.sq && songDlWeight >= 4 && songPlWeight < 4) {
            const desc = `${Math.round(songDetail.sq.br / 1000)}k\t${fileSizeDesc(songDetail.sq.size)}\t${songDetail.sq.sr / 1000}kHz`;
            const level = 'lossless';
            this.createDLRow(desc, level, channel);
            this.createULRow(desc, level, channel);
          }
          if (songDetail.h && songDlWeight >= 3 && songPlWeight < 3) {
            const desc = `${Math.round(songDetail.h.br / 1000)}k\t${fileSizeDesc(songDetail.h.size)}`;
            const level = 'exhigh';
            this.createDLRow(desc, level, channel);
            this.createULRow(desc, level, channel);
          }
          if (songDetail.m && songDlWeight >= 2 && songPlWeight < 2) {
            const desc = `${Math.round(songDetail.m.br / 1000)}k\t${fileSizeDesc(songDetail.m.size)}`;
            const level = 'higher';
            this.createDLRow(desc, level, channel);
            this.createULRow(desc, level, channel);
          }
        ...
        if (songDetail.jm && songPlWeight >= 7) {
          const desc = `${Math.round(songDetail.jm.br / 1000)}k\t${fileSizeDesc(songDetail.jm.size)}\t${songDetail.jm.sr / 1000}kHz`;
          const level = 'jymaster';
          this.createDLRow(desc, level, channel);
          this.createULRow(desc, level, channel);
        }
        if (songDetail.db && songPlWeight >= 7) {
          const desc = `${Math.round(songDetail.db.br / 1000)}k\t${fileSizeDesc(songDetail.db.size)}\t${songDetail.db.sr / 1000}kHz`;
          const level = 'dolby';
          this.createDLRow(desc, level, channel);
          this.createULRow(desc, level, channel);
        }
        if (songDetail.sk && songPlWeight >= 7) {
          const desc = `${Math.round(songDetail.sk.br / 1000)}k\t${fileSizeDesc(songDetail.sk.size)}\t${songDetail.sk.sr / 1000}kHz`;
          const level = 'sky';
          this.createDLRow(desc, level, channel);
          this.createULRow(desc, level, channel);
        }
        if (songDetail.je && songPlWeight >= 4) {
          const desc = `${Math.round(songDetail.je.br / 1000)}k\t${fileSizeDesc(songDetail.je.size)}\t${songDetail.je.sr / 1000}kHz`;
        ...
        if (songDetail.l && songPlWeight >= 1) {
          const desc = `${Math.round(songDetail.l.br / 1000)}k ${fileSizeDesc(songDetail.l.size)}`;
          ...
          if (creative.resources) {
            for (let resource of creative.resources) {
              ...
              if (resource.uiElement.mainTitle) {
                let IsLink =
                  resource.action?.clickAction?.action === 1 &&
                  resource.action?.clickAction?.targetUrl.startsWith('https://');
                let mainTitleItem = IsLink
                  ? this.createButton(resource.uiElement.mainTitle.title)
                  : this.createText(resource.uiElement.mainTitle.title);
                if (IsLink) {
  ...
  createULRow(desc, level, channel) {
    ...
    const data =
      channel === 'dl'
        ? { id: this.songId, level: level, encodeType: 'mp3' }
        : { ids: JSON.stringify([this.songId]), level: level, encodeType: 'mp3' };
