...
class BrowserBuffer {
  ...
  readUInt32BE(offset) {
    return (
      this._u8[offset] * 0x1000000 + ((this._u8[offset + 1] << 16) | (this._u8[offset + 2] << 8) | this._u8[offset + 3])
    );
  }
  ...
  readUInt32LE(offset) {
    return (
      this._u8[offset] |
      (this._u8[offset + 1] << 8) |
      (this._u8[offset + 2] << 16) |
      ((this._u8[offset + 3] << 24) >>> 0)
    );
  }
...
class MetaFlac {
  ...
  importPictureFromBuffer(picture) {
    const arr =
      picture instanceof BrowserBuffer
